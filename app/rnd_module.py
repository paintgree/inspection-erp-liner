...
DEFAULT_SPECIMEN_RULE = {
    ...
    "effective_length": "Active section defined by the method or fixture arrangement.",
    "end_allowance_each_side": "Provide enough extra length for sealing, gripping, fittings, and trimming.",
    "total_length_formula": "Total cut length = effective test length + left end allowance + right end allowance + trimming margin",
    "conditioning": {
        "required": "depends",
        "notes": "Follow the test-specific requirement for temperature, fluid, soak time, and pre-test stabilization.",
        "reconditioning": "Only when the method or engineering basis requires it after end preparation or specimen handling.",
    },
    "preparation_steps": [
        "Assign specimen ID and link it to the pipe build, batch, and qualification program.",
        "Cut the specimen square and record the total cut length before any end preparation.",
        "Define and mark the effective test length clearly so the active section is known during evaluation.",
        "Prepare end regions to suit the test fixture, seals, or end fittings without damaging the pipe body.",
        "Record conditioning status, temperature basis, and any pre-test soak or stabilization details.",
    ],
    "release_checks": [
        "Specimen ID assigned and traceability confirmed",
        "Total cut length measured and recorded",
        "Effective test length defined and marked",
        "End preparation completed and visually acceptable",
        "No visible damage, kinks, scoring, or unintended deformation",
        "Conditioning / soak requirement reviewed before test start",
    ],
}

SPECIMEN_PREP_RULES = {
    "MPR_REG": {
        "min_specimens": "18+ target for full nonmetallic PFR regression set",
        "effective_length": "The free pressurized section between end terminations that is intended to fail in the pipe body, not in the fittings.",
        "end_allowance_each_side": "Use sufficient length for end terminations and sealing; keep the effective section isolated from fitting effects.",
        "conditioning": {
            "required": True,
            "notes": "Condition specimens at the intended test temperature and stabilize before pressurization.",
            "reconditioning": "Reconditioning depends on internal method and whether specimen preparation or transport disturbed the required thermal condition.",
        },
    },
    "CYCLIC_REG": {
        "min_specimens": "18+ target for cyclic qualification route",
        "effective_length": "The active pressure-cycling section between terminations, with body failure expected away from the ends.",
        "end_allowance_each_side": "Provide enough length for end fittings, seals, and instrumentation without disturbing the active cyclic zone.",
        "conditioning": {
            "required": True,
            "notes": "Condition to the cyclic test temperature before cycling begins.",
            "reconditioning": "Review if specimens are interrupted, reassembled, or exposed outside the required thermal basis before restart.",
        },
    },
    "PV_1000H": {
        "min_specimens": "2",
        "effective_length": "Constant-pressure active section between end terminations for the full 1000 h confirmation duration.",
        "conditioning": {
            "required": True,
            "notes": "Condition to the chosen confirmation test temperature before loading.",
            "reconditioning": "Normally not needed unless the method or handling history requires renewed conditioning.",
        },
    },
    "TEMP_ELEV": {
        "effective_length": "Active section that sees elevated-temperature exposure during confirmation.",
        "conditioning": {
            "required": True,
            "notes": "Elevated-temperature soak and stabilization are part of the setup basis.",
            "reconditioning": "Review only if exposure is interrupted before the actual test starts.",
        },
    },
    "TEMP_CYCLE": {
        "min_specimens": "2",
        "effective_length": "Section exposed to the full thermal-cycling envelope while restrained by the intended fixture arrangement.",
        "conditioning": {
            "required": True,
            "notes": "Precondition to the start temperature and record the cycling envelope and dwell basis.",
            "reconditioning": "Not normally required unless the sequence is broken or the specimen is removed from the controlled environment.",
        },
    },
    "RAPID_DECOMP": {
        "effective_length": "Pressure-containing section that sees decompression exposure under the selected gas or multiphase condition.",
        "conditioning": {
            "required": True,
            "notes": "Condition per the decompression protocol and gas exposure basis before pressure release.",
            "reconditioning": "Depends on the decompression sequence and whether the specimen is reused across stages.",
        },
    },
    "OPERATING_MBR": {
        "min_specimens": "2",
        "effective_length": "Full bend-controlled section needed to demonstrate the required bend radius and respooling behavior.",
        "end_allowance_each_side": "Add sufficient handling length outside the active bend zone so grips and fixtures do not distort the required curvature region.",
    },
    "AXIAL_LOAD": {
        "min_specimens": "2",
        "effective_length": "Axially loaded section between grips or end fittings where strain and load transfer are intended to occur.",
        "end_allowance_each_side": "Provide enough engaged length for the axial fixture to develop load without local slippage.",
    },
    "CRUSH": {
        "min_specimens": "3",
        "effective_length": "Loaded span used in the crush setup under the selected support and loading geometry.",
    },
    "LAOT": {
        "min_specimens": "2",
        "effective_length": "Test section exposed to the minimum-temperature condition and subsequent evaluation step.",
        "conditioning": {
            "required": True,
    ...
    "IMPACT": {
        "min_specimens": "2",
        "effective_length": "Section located under the prescribed impact location and support span.",
        "conditioning": {
            "required": True,
            "notes": "Condition to the required test temperature before impact exposure.",
            "reconditioning": "Usually not needed unless the impact protocol is broken between conditioning and testing.",
        },
    },
    "TEC": {
        "min_specimens": "2",
        "effective_length": "Measured gauge length used for thermal expansion evaluation under controlled temperature change.",
        "conditioning": {
            "required": True,
            "notes": "Stabilize at each temperature point before reading expansion or contraction.",
            "reconditioning": "Not generally required; use staged stabilization instead.",
        },
    },
    "GROWTH": {
        "min_specimens": "2",
        "effective_length": "Measured pipe body length used to monitor pressure-driven dimensional change.",
        "conditioning": {
            "required": True,
            "notes": "Stabilize at the required temperature before pressurization and dimensional reading.",
            "reconditioning": "Only if the test basis requires renewed stabilization after interruption.",
        },
    },
    "BURST_QUALIFICATION": {
        "min_specimens": "5 internal design-screen specimens",
        "effective_length": "The pipe body section between end fittings where rupture is expected to occur away from fittings and seals.",
        "end_allowance_each_side": "Use enough allowance for burst end fittings, seals, and grip engagement. A simple rule is not enough; the fixture design governs the allowance.",
        "conditioning": {
            "required": False,
            "notes": "Conditioning is only required when the burst screen is intentionally run at elevated or controlled temperature.",
            "reconditioning": "Not normally required for ambient burst screening. Review if specimens are reheated, cooled, or stored after preparation.",
        },
    },
}


def get_specimen_prep(test_code: str, specimen_requirement: str = "") -> dict:
    base = {
        "min_specimens": DEFAULT_SPECIMEN_RULE["min_specimens"],
        "effective_length": DEFAULT_SPECIMEN_RULE["effective_length"],
        "end_allowance_each_side": DEFAULT_SPECIMEN_RULE["end_allowance_each_side"],
        "total_length_formula": DEFAULT_SPECIMEN_RULE["total_length_formula"],
        "conditioning": dict(DEFAULT_SPECIMEN_RULE["conditioning"]),
        "preparation_steps": list(DEFAULT_SPECIMEN_RULE["preparation_steps"]),
        "release_checks": list(DEFAULT_SPECIMEN_RULE["release_checks"]),
    }
    override = SPECIMEN_PREP_RULES.get((test_code or "").upper(), {})
    for key, value in override.items():
        if isinstance(value, dict) and key in base and isinstance(base[key], dict):
            base[key].update(value)
        else:
            base[key] = value
    if specimen_requirement and base.get("min_specimens") == DEFAULT_SPECIMEN_RULE["min_specimens"]:
        base["min_specimens"] = specimen_requirement
    return base
...
def rnd_test_detail(program_id: int, test_id: int, request: Request, session: Session = Depends(get_session), user: User = Depends(_require_user)):
    ...
    if not test or test.program_id != program_id:
        raise HTTPException(404, 'Qualification test not found')
    prep = get_specimen_prep(test.code, test.specimen_requirement)
    return TEMPLATES.TemplateResponse(
        request=request,
