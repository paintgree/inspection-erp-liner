...
def _test_rulebook(program: RndQualificationProgram, test: RndQualificationTest) -> dict:
    ...
    data["specimen_prep"] = _specimen_prep_rulebook(program, test)
    return data




def _specimen_prep_rulebook(program: RndQualificationProgram, test: RndQualificationTest) -> dict:
    base = {
        "overview": "Prepare only specimens that are fully traceable to the qualified construction. Keep preparation method, active test length, end condition, and conditioning history consistent within the same test set.",
        "count_text": test.specimen_requirement or "Use the program-specific specimen count.",
        "length_basis": "Length is not a random cut length. It should follow the active test setup, free test section, fixture/end-fitting arrangement, and the need to avoid end effects corrupting the result.",
        "conditioning": {
            "required": "Review required",
            "reconditioning": "Engineering decision",
            "guidance": [
                "Check whether the method requires temperature soak, fluid exposure, or time stabilization before test start.",
                "If cutting, handling, or transport changed the specimen condition, record whether reconditioning is needed before release to test.",
                "Do not start the test until the conditioning state is recorded in the raw record.",
            ],
        },
        "checkpoints": [
            "Specimen ID assigned and traceable to batch/lot.",
            "Nominal size and build match the selected qualification route.",
            "Ends prepared consistently with the test setup.",
            "Visible damage, ovality, dents, or handling defects checked before release.",
            "Conditioning status recorded before test start.",
        ],
        "release_note": "The specimen should be released to test only when preparation, conditioning status, and traceability are all clear in the record.",
    }
    mapping = {
        "MPR_REG": {
            "overview": "Build a distributed regression set. Keep specimen construction, cut method, fittings, and preparation practice consistent across pressure groups so the dataset reflects pipe behavior, not preparation noise.",
            "count_text": "PFR route target: 18 valid long-term points, spread across several pressure levels and time windows.",
            "length_basis": "Use a consistent active test length and end-fitting arrangement across the regression matrix. The chosen length must minimize end effects and allow the failure to represent the pipe body rather than the fittings.",
            "conditioning": {
                "required": "Usually yes",
                "reconditioning": "Depends on prep method and method basis",
                "guidance": [
                    "Hold all specimens to the same pre-test temperature basis before pressurization.",
                    "If the lab method or internal procedure requires soak time, record soak duration and medium for every specimen.",
                    "If a specimen is reworked or stored after preparation, record whether reconditioning was applied before restart.",
                ],
            },
            "checkpoints": [
                "Specimen assigned to a planned pressure band / target failure window.",
                "Length, end-fitting arrangement, and setup consistent with the regression method.",
                "Conditioning/soak record completed before test start.",
                "Start pressure, temperature basis, and witness/lab reference ready in the raw sheet.",
            ],
            "release_note": "Do not release a regression specimen until its pressure band, temperature basis, and conditioning status are recorded. Mixed preparation practices can distort the slope and confidence line.",
        },
        "PV_1000H": {
            "overview": "Prepare variant specimens exactly as the PV construction to confirm family coverage against the parent PFR relationship.",
            "count_text": "PV route: 2 specimens for the 1000 h confirmation test.",
            "length_basis": "Use the same general setup logic as the parent method, with enough active length to avoid fitting-dominated results.",
            "conditioning": {
                "required": "Yes, if defined by the test method",
                "reconditioning": "Only if the specimen condition changed after preparation",
                "guidance": [
                    "Keep the variant construction clearly identified against the parent PFR basis.",
                    "Record the pressure relationship used for the PV confirmation.",
                ],
            },
        },
        "RAPID_DECOMP": {
            "overview": "Prepare specimens representative of gas/multiphase service. Handling damage and poor sealing can invalidate the decompression result.",
            "count_text": test.specimen_requirement or "Use the route-specific specimen count.",
            "length_basis": "Use the decompression fixture/setup basis so pressure release and observation are representative.",
            "conditioning": {
                "required": "Yes",
                "reconditioning": "Normally no, unless the method specifically calls for it",
                "guidance": [
                    "Stabilize specimens to the defined temperature and gas/fluid exposure condition before the decompression cycle.",
                    "Record any visible damage before cycling so new damage can be distinguished after test.",
                ],
            },
        },
        "CYCLIC_REG": {
            "overview": "Prepare cyclic specimens with the same discipline as static regression, but keep cycle-duty grouping and setup consistency because fatigue response is sensitive to preparation differences.",
            "count_text": "Cyclic route target: 18 cyclic points unless engineering review defines a narrower development stage.",
            "length_basis": "Use the cyclic fixture/setup basis and keep the active length identical within each group so comparisons remain meaningful.",
            "conditioning": {
                "required": "Usually yes",
                "reconditioning": "Depends on stoppage / storage between preparation and start",
                "guidance": [
                    "Record cycle range, mean pressure, and temperature basis before release to test.",
                    "If a specimen pauses after setup, document whether reconditioning was needed before cycling resumes.",
                ],
            },
        },
    }
    data = {**base, **mapping.get(test.code, {})}
    return data


def _current_prep_panel(program: RndQualificationProgram, matrix_rows: list[dict], flow: dict) -> dict:
    current_code = {"materials": "MPR_REG", "burst": "MPR_REG", "regression": "MPR_REG", "review": "MPR_REG"}.get(flow["current"], "MPR_REG")
    selected = None
    for row in matrix_rows:
        if row["test"].code == current_code:
            selected = row
            break
    if not selected and matrix_rows:
        selected = matrix_rows[0]
    if not selected:
        return {"title": "No active test yet", "prep": None}
    return {"title": selected["test"].title, "test": selected["test"], "prep": selected["rulebook"].get("specimen_prep")}
...
def _program_context(session: Session, program_id: int) -> dict:
    ...
    return {
        ...
        "current_prep_panel": _current_prep_panel(program, matrix_rows, flow),
