...
# =========================
# R&D QUALIFICATION MODULE
# =========================

class RndQualificationProgram(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)

    program_code: str = Field(default="", index=True, unique=True)
    ...
    pfr_or_pv: str = Field(default="PFR", index=True)  # PFR / PV
    parent_program_id: Optional[int] = Field(default=None, index=True)
    pfr_reference_code: str = Field(default="")

    service_medium: str = Field(default="WATER")
    service_factor: float = Field(default=1.0)
    intended_service: str = Field(default="")

    product_family: str = Field(default="")
...
class RndQualificationTest(SQLModel, table=True):
    ...
    program_id: int = Field(foreign_key="rndqualificationprogram.id", index=True)
    ...
    specimen_count: Optional[int] = Field(default=None)

    applicability: str = Field(default="CORE")
    scope_tag: str = Field(default="BOTH")  # BOTH / PFR / PV / CUSTOM
    source_standard: str = Field(default="API_15S")

    status: str = Field(default="PLANNED", index=True)
    result_summary: str = Field(default="")

    guidance_when_required_override: str = Field(default="")
    guidance_specimen_count_override: str = Field(default="")
    guidance_api_clause_override: str = Field(default="")
    guidance_external_standard_override: str = Field(default="")
    guidance_conditioning_required_override: str = Field(default="")
    guidance_conditioning_steps_override: str = Field(default="")
    guidance_core_process_override: str = Field(default="")
    guidance_acceptance_override: str = Field(default="")
    guidance_retest_logic_override: str = Field(default="")
    guidance_practical_notes_override: str = Field(default="")
...
class RndQualificationSpecimen(SQLModel, table=True):
    ...
    program_id: int = Field(foreign_key="rndqualificationprogram.id", index=True)
    test_id: Optional[int] = Field(default=None, foreign_key="rndqualificationtest.id", index=True)
    ...
    pressure_mpa: float = Field(default=0.0, index=True)
...
class RndMaterialQualification(SQLModel, table=True):
    ...
    component: str = Field(default="", index=True)  # LINER / REINFORCEMENT / MATRIX / COVER / FITTING
    ...
    material_family: str = Field(default="", index=True)
...
class RndMaterialTestRecord(SQLModel, table=True):
    ...
    program_id: int = Field(foreign_key="rndqualificationprogram.id", index=True)
    material_id: int = Field(foreign_key="rndmaterialqualification.id", index=True)
...
class RndAttachmentRegister(SQLModel, table=True):
    ...
    source_mode: str = Field(default="UPLOAD")
...
class RndQualificationProgram(SQLModel, table=True):
    ...
    maot_c: float = Field(default=0.0)
    laot_c: float = Field(default=0.0)
    pfr_or_pv: str = Field(default="PFR", index=True)
    parent_program_id: Optional[int] = Field(default=None, index=True)
    pfr_reference_code: str = Field(default="")
    service_medium: str = Field(default="WATER")
    service_factor: float = Field(default=1.0)
...
class RndQualificationTest(SQLModel, table=True):
    ...
    specimen_count: Optional[int] = Field(default=None)
    applicability: str = Field(default="CORE")
    scope_tag: str = Field(default="BOTH")
    source_standard: str = Field(default="API_15S")
    status: str = Field(default="PLANNED", index=True)
    result_summary: str = Field(default="")
    guidance_when_required_override: str = Field(default="")
...
class RndMaterialQualification(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    program_id: int = Field(foreign_key="rndqualificationprogram.id", index=True)
    component: str = Field(default="", index=True)
    material_name: str = Field(default="")
    supplier_name: str = Field(default="")
    manufacturer_name: str = Field(default="")
    grade_name: str = Field(default="")
