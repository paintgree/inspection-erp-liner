...
def _burst_basis(program: RndQualificationProgram) -> dict:
    threshold = _burst_threshold(program)
    return {
        'threshold_mpa': threshold,
        'formula': 'Planning screen = target NPR / Fd',
        'equation': f"{program.npr_mpa:.3f} / {DESIGN_FACTOR_NONMETALLIC:.2f} = {threshold:.3f} MPa",
        'fd': DESIGN_FACTOR_NONMETALLIC,
        'summary': 'This value is used in the app as an internal burst screening gate so early burst results can be sanity-checked against the claimed pressure target.',
        'standard_basis': 'API 15S defines NPR as not exceeding MPR, and for nonmetallic reinforced PFRs it defines MPR from the long-term regression basis using LCL at RCRT multiplied by Fd. That means the real qualification backbone is regression, not this screen value.',
        'caution': 'So 14.925 MPa is not a final API acceptance value by itself for your nonmetallic PFR. It is a conservative screen derived from the claimed 10 MPa and the 0.67 design factor.',
    }
