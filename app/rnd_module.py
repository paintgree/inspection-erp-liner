...
_CUSTOM_TEST_LIBRARY = {
    'BURST': {
        'title': 'Burst qualification / proof test',
        'category': 'Pressure Integrity',
        'phase': 'Execution',
        'execution_mode': 'numeric_and_result',
        'result_mode': 'pass_fail_with_note',
        'description': 'Short-time burst or proof pressure confirmation for qualification or client witness testing.',
        'guidance_text': 'Prepare traceable specimens, confirm dimensions and effective length, test at the required temperature, capture the pressure-time chart, and record failure mode, burst pressure, and visual condition.',
        'formula_text': 'Minimum burst threshold can be linked to qualification basis or client specification.',
        'required_uploads': ['REPORT', 'CHART', 'FULL_LENGTH_PHOTO', 'SIDE_A_PHOTO', 'SIDE_B_PHOTO'],
        'input_fields': [
            {'key': 'pressure_mpa', 'label': 'Burst pressure (MPa)', 'type': 'number'},
            {'key': 'temperature_c', 'label': 'Test temperature (°C)', 'type': 'number'},
            {'key': 'failure_mode', 'label': 'Failure mode', 'type': 'text'},
        ],
        'samples_required': '5 specimens unless the client basis states otherwise.',
    },
    'PRESSURE_HOLD': {
        'title': 'Pressure hold / soak test',
        'category': 'Pressure Integrity',
        'phase': 'Execution',
        'execution_mode': 'numeric_and_result',
        'result_mode': 'pass_fail_with_note',
        'description': 'Constant pressure hold qualification or project witness test.',
        'guidance_text': 'Record pressure, temperature, exposure duration, leakage observations, and post-test condition.',
        'formula_text': 'Pressure and duration follow the selected qualification or client basis.',
        'required_uploads': ['REPORT', 'PRESSURE_LOG', 'TEMPERATURE_LOG', 'SPECIMEN_PHOTO'],
        'input_fields': [
            {'key': 'pressure_mpa', 'label': 'Hold pressure (MPa)', 'type': 'number'},
            {'key': 'failure_hours', 'label': 'Duration (hours)', 'type': 'number'},
            {'key': 'temperature_c', 'label': 'Temperature (°C)', 'type': 'number'},
        ],
        'samples_required': 'As defined by the selected program.',
    },
    'BEND': {
        'title': 'Bending / MBR qualification',
        'category': 'Mechanical',
        'phase': 'Execution',
        'execution_mode': 'checklist_and_result',
        'result_mode': 'pass_fail_with_note',
        'description': 'Operating, handling, or respooling bending validation.',
        'guidance_text': 'Set the specimen at the required radius, document the fixture arrangement, note any damage or leakage, and record any required follow-on proof test.',
        'formula_text': 'Use the approved operating, handling, or respooling radius basis.',
        'required_uploads': ['SETUP_PHOTO', 'RESULT_SHEET', 'SPECIMEN_PHOTO'],
        'input_fields': [
            {'key': 'notes', 'label': 'Bend / setup notes', 'type': 'text'},
        ],
        'samples_required': '2 specimens unless the project basis states otherwise.',
    },
    'TEMPERATURE': {
        'title': 'Temperature qualification',
        'category': 'Thermal',
        'phase': 'Execution',
        'execution_mode': 'numeric_and_result',
        'result_mode': 'pass_fail_with_note',
        'description': 'Elevated, low-temperature, or thermal-cycle validation.',
        'guidance_text': 'Record soak conditions, applied temperature range, duration or cycle count, and final leakage / damage observations.',
        'formula_text': 'Thermal exposure follows the selected standard or client specification.',
        'required_uploads': ['REPORT', 'TEMPERATURE_LOG', 'SETUP_PHOTO'],
        'input_fields': [
            {'key': 'temperature_c', 'label': 'Temperature (°C)', 'type': 'number'},
            {'key': 'failure_cycles', 'label': 'Cycle count', 'type': 'number'},
        ],
        'samples_required': 'As required by the selected route.',
    },
    'VISUAL': {
        'title': 'Visual / witness inspection',
        'category': 'Evidence Review',
        'phase': 'Evidence Review',
        'execution_mode': 'text_confirmation',
        'result_mode': 'text_summary',
        'description': 'Auditor or client witness verification step.',
        'guidance_text': 'Record what was checked, who witnessed it, and which evidence documents support the decision.',
        'formula_text': '',
        'required_uploads': ['CHECKLIST', 'PHOTO', 'SIGNED_FORM'],
        'input_fields': [
            {'key': 'notes', 'label': 'Witness / inspection note', 'type': 'text'},
        ],
        'samples_required': 'As required by the checklist.',
    },
}
...
def _phase_for_code(code: str) -> str:
    mapping = {
        ...
        'PV_1000H': 'Execution',
        'TEMP_ELEV': 'Execution',
        'TEMP_CYCLE': 'Execution',
        'RAPID_DECOMP': 'Execution',
        'OPERATING_MBR': 'Execution',
        'AXIAL_LOAD': 'Execution',
        'CRUSH': 'Execution',
        'LAOT': 'Execution',
        'IMPACT': 'Execution',
        'TEC': 'Execution',
        'GROWTH': 'Execution',
        'CYCLIC_REG': 'Execution',
    }
    return mapping.get((code or '').upper(), 'Execution')


def _test_payload(test: RndQualificationTest) -> dict:
    raw = (test.result_summary or '').strip()
    if raw.startswith('__RNDTESTJSON__'):
        try:
            return json.loads(raw[len('__RNDTESTJSON__'):])
        except Exception:
            return {}
    return {}


def _test_plain_summary(test: RndQualificationTest) -> str:
    payload = _test_payload(test)
    if payload:
        return payload.get('summary', '') or ''
    return test.result_summary or ''


def _save_test_payload(test: RndQualificationTest, payload: dict) -> None:
    test.result_summary = '__RNDTESTJSON__' + json.dumps(payload, ensure_ascii=False)
    _touch_row(test)


def _build_test_plan(test: RndQualificationTest) -> dict:
    payload = _test_payload(test)
    guidance = get_test_guidance(test.code)
    required_uploads = payload.get('required_uploads') or _required_attachment_types(test.code)
    input_fields = payload.get('input_fields') or []
    category = payload.get('category') or ('Custom Qualification' if (test.applicability or '').upper() == 'CUSTOM' else 'API 15S Qualification')
    phase = payload.get('phase') or _phase_for_code(test.code)
    execution_mode = payload.get('execution_mode') or ('text_confirmation' if (test.applicability or '').upper() == 'CUSTOM' else 'numeric_and_result')
    result_mode = payload.get('result_mode') or 'pass_fail_with_note'
    return {
        'phase': phase,
        'category': category,
        ...
        'guidance_text': payload.get('guidance_text') or test.description or guidance.get('core_process', [''])[0],
        'formula_text': payload.get('formula_text') or '',
        'samples_required': payload.get('samples_required') or test.specimen_requirement or guidance.get('specimen_count') or 'As required',
        'acceptance_note': payload.get('acceptance_note') or ' / '.join(guidance.get('acceptance', [])),
        'conditioning_note': payload.get('conditioning_note') or guidance.get('conditioning_required') or '',
        'summary': _test_plain_summary(test),
    }


def _checklist_rows(program: RndQualificationProgram, tests: list[RndQualificationTest], specimens: list[RndQualificationSpecimen], attachments: list[RndAttachmentRegister]) -> list[dict]:
    rows = []
    for test in tests:
        plan = _build_test_plan(test)
        test_specs = [s for s in specimens if s.test_id == test.id]
        test_attachments = [a for a in attachments if a.test_id == test.id]
        evidence = _evidence_status(test.code, test_attachments)
        if _test_payload(test).get('required_uploads'):
            required = [item.upper() for item in plan['required_uploads']]
            uploaded = {(a.document_type or '').upper() for a in test_attachments}
            evidence = {
                'required': required,
                'rows': [{'document_type': item, 'present': item in uploaded} for item in required],
                'missing': [item for item in required if item not in uploaded],
                'complete': all(item in uploaded for item in required) if required else True,
            }
        specimen_target = _extract_first_int(plan['samples_required'])
        inputs_complete = len(test_specs) >= (specimen_target or 1) if plan['execution_mode'] != 'text_confirmation' else len(test_specs) > 0 or bool(plan['summary'])
        results_complete = bool(plan['summary']) or (test.status or '').upper() in {'PASSED', 'FAILED', 'WAIVED'}
        rows.append({
            'test': test,
            'plan': plan,
            'specimens': test_specs,
            'attachments': test_attachments,
            'evidence': evidence,
            'specimen_target': specimen_target,
...
def _phase_summary(checklist_rows: list[dict], program: RndQualificationProgram, materials: list[RndMaterialQualification], attachments: list[RndAttachmentRegister]) -> list[dict]:
    material_state = _material_screening_state(materials)
    definition_done = bool((program.program_code or '').strip() and (program.title or '').strip() and (program.intended_service or '').strip())
    execution_total = len(checklist_rows)
    execution_done = sum(1 for row in checklist_rows if (row['test'].status or '').upper() in {'PASSED', 'WAIVED'})
    evidence_total = len(checklist_rows)
    evidence_done = sum(1 for row in checklist_rows if row['evidence']['complete'])
    closure_ready = definition_done and material_state['complete'] and execution_done == execution_total and evidence_done == evidence_total and execution_total > 0
    rows = [
        {'name': 'Definition', 'status': 'COMPLETE' if definition_done else 'OPEN', 'detail': 'Program basis, claim, and client scope are defined.'},
        {'name': 'Material Control', 'status': 'COMPLETE' if material_state['complete'] else 'OPEN', 'detail': f"{sum(1 for r in material_state['rows'] if r['ready'])}/{len(material_state['rows'])} material records are fully traceable."},
        {'name': 'Test Planning', 'status': 'COMPLETE' if execution_total > 0 else 'OPEN', 'detail': f'{execution_total} checklist tests are loaded in the program.'},
        {'name': 'Execution', 'status': 'COMPLETE' if execution_total and execution_done == execution_total else 'OPEN', 'detail': f'{execution_done}/{execution_total} tests are closed as passed or waived.'},
        {'name': 'Evidence Review', 'status': 'COMPLETE' if evidence_total and evidence_done == evidence_total else 'OPEN', 'detail': f'{evidence_done}/{evidence_total} tests have complete evidence checklists.'},
        {'name': 'Closure', 'status': 'READY' if closure_ready else 'OPEN', 'detail': 'Close only when the checklist, evidence, and results are complete.'},
    ]
    return rows
...
def rnd_dashboard(request: Request, session: Session = Depends(get_session), user: User = Depends(_require_user)):
    ...
    for program in programs:
        ...
        attachments = session.exec(
            ...
            .where(RndAttachmentRegister.program_id == program.id)
        ...
        checklist_rows = _checklist_rows(program, tests, specimens, attachments)
        phases = _phase_summary(checklist_rows, program, materials, attachments)
        dashboard.append({
            'program': program,
            'materials': _material_screening_state(materials),
            'burst': _burst_state(program, specimens),
            'checklist_rows': checklist_rows,
            'phases': phases,
            'phase_complete_count': sum(1 for row in phases if row['status'] in {'COMPLETE', 'READY'}),
            'completion_pct': int(round((sum(1 for row in checklist_rows if (row['test'].status or '').upper() in {'PASSED', 'WAIVED'}) / len(checklist_rows)) * 100)) if checklist_rows else 0,
        })
...
def rnd_program_view(program_id: int, request: Request, session: Session = Depends(get_session), user: User = Depends(_require_user)):
    ...
    burst = _burst_state(program, specimens)
    answers = _program_answers(program)
    return TEMPLATES.TemplateResponse(request,'rnd_program_view.html', {
        'request': request,
        'user': user,
        'program': program,
        'tests': tests,
        ...
        'burst': burst,
        'answers': answers,
        'custom_library': sorted(_CUSTOM_TEST_LIBRARY.items()),
    })
...
@router.post('/qualifications/{program_id}/basis')
def rnd_update_program_basis(
    program_id: int,
    title: str = Form(...),
    program_code: str = Form(...),
    ...
    npr_mpa: float = Form(0.0),
    maot_c: float = Form(0.0),
    ...
    program.intended_service = intended_service or ''
    program.qualification_standard = qualification_standard or 'API 15S R3'
    program.pfr_or_pv = (pfr_or_pv or 'PFR').strip().upper()
    program.nominal_size_in = nominal_size_in or program.nominal_size_in
    program.npr_mpa = npr_mpa or program.npr_mpa
    program.maot_c = maot_c if maot_c is not None else program.maot_c
    program.laot_c = laot_c if laot_c is not None else program.laot_c
    program.liner_material = liner_material or program.liner_material
    program.reinforcement_material = reinforcement_material or program.reinforcement_material
    program.cover_material = cover_material or program.cover_material
    answers = _program_answers(program)
    answers.update({
...
def rnd_update_test(
    ...
    result_mode: str = Form(''),
    guidance_text: str = Form(''),
    formula_text: str = Form(''),
    acceptance_note: str = Form(''),
    conditioning_note: str = Form(''),
    samples_required: str = Form(''),
    required_uploads_csv: str = Form(''),
    session: Session = Depends(get_session),
    user: User = Depends(_require_user),
):
    ...
    payload = _test_payload(test)
    payload.update({
        'summary': result_summary or payload.get('summary', ''),
        'category': category or payload.get('category', ''),
        'phase': phase or payload.get('phase', ''),
        'execution_mode': execution_mode or payload.get('execution_mode', ''),
        'result_mode': result_mode or payload.get('result_mode', ''),
        'guidance_text': guidance_text or payload.get('guidance_text', ''),
        'formula_text': formula_text or payload.get('formula_text', ''),
        'acceptance_note': acceptance_note or payload.get('acceptance_note', ''),
        'conditioning_note': conditioning_note or payload.get('conditioning_note', ''),
        'samples_required': samples_required or payload.get('samples_required', ''),
    })
    uploads = _parse_csv_items(required_uploads_csv)
    if uploads:
        payload['required_uploads'] = [item.upper() for item in uploads]
    if payload:
        _save_test_payload(test, payload)
    else:
...
def rnd_add_custom_test(
    program_id: int,
    title: str = Form(''),
    code: str = Form('OTHER'),
    description: str = Form(''),
    specimen_requirement: str = Form('As required'),
    clause_ref: str = Form('CUSTOM'),
    common_test_type: str = Form('OTHER'),
    category: str = Form('Custom Qualification'),
    phase: str = Form('Execution'),
    execution_mode: str = Form('numeric_and_result'),
    result_mode: str = Form('pass_fail_with_note'),
    guidance_text: str = Form(''),
    formula_text: str = Form(''),
    required_uploads_csv: str = Form(''),
    session: Session = Depends(get_session),
    ...
    common_key = (common_test_type or 'OTHER').strip().upper()
    preset = _CUSTOM_TEST_LIBRARY.get(common_key, {})
    final_title = (title or '').strip() or preset.get('title') or f'Custom Test {next_order}'
    norm_code = (code or common_key or 'OTHER').strip().upper()
    if norm_code == 'OTHER':
        norm_code = f'OTHER_{next_order}'
    row = RndQualificationTest(
        program_id=program_id,
        sort_order=next_order,
        clause_ref=(clause_ref or 'CUSTOM').strip(),
        code=norm_code,
        title=final_title,
        description=(description or '').strip() or preset.get('description', ''),
        specimen_requirement=(specimen_requirement or '').strip() or preset.get('samples_required', 'As required'),
        applicability='CUSTOM',
        status='PLANNED',
    )
    payload = {
        'summary': '',
        'category': category or preset.get('category', 'Custom Qualification'),
        'phase': phase or preset.get('phase', 'Execution'),
        'execution_mode': execution_mode or preset.get('execution_mode', 'numeric_and_result'),
        'result_mode': result_mode or preset.get('result_mode', 'pass_fail_with_note'),
        'guidance_text': guidance_text or preset.get('guidance_text', ''),
        'formula_text': formula_text or preset.get('formula_text', ''),
        'required_uploads': [item.upper() for item in (_parse_csv_items(required_uploads_csv) or preset.get('required_uploads', []))],
        'samples_required': (specimen_requirement or '').strip() or preset.get('samples_required', 'As required'),
        'input_fields': preset.get('input_fields', []),
    }
    _save_test_payload(row, payload)
    session.add(row)
    _touch_program(program)
    session.add(program)
    session.commit()
...
def rnd_report_view(program_id: int, report_type: str, request: Request, session: Session = Depends(get_session), user: User = Depends(_require_user)):
    ...
    burst = _burst_state(program, specimens)
    completion_pct = int(round((sum(1 for row in checklist_rows if (row['test'].status or '').upper() in {'PASSED', 'WAIVED'}) / len(checklist_rows)) * 100)) if checklist_rows else 0
    return TEMPLATES.TemplateResponse(request, 'rnd_report.html', {
        'request': request,
        'user': user,
        'program': program,
        'tests': tests,
