...
def fill_mrr_f02_docx_bytes(*, lot, inspection, receiving, docs: list) -> bytes:
    ...
    for t in doc.tables:
        ...
        for i, it in enumerate(items):
            ...
            if len(row) > 0: _set_cell_text(row[0], getv("item"))
            if len(row) > 1: _set_cell_text(row[1], getv("po_desc"))
            if len(row) > 2: _set_cell_text(row[2], getv("size"))
            if len(row) > 3: _set_cell_text(row[3], getv("type"))
    ...
    for t in doc.tables:
        ...
        for i, it in enumerate(visual_rows):
            ...
            if len(row) > 0: _set_cell_text(row[0], getv("heat_batch"))
            if len(row) > 1: _set_cell_text(row[1], getv("flange_id"))
            if len(row) > 2: _set_cell_text(row[2], getv("surface"))
            if len(row) > 3: _set_cell_text(row[3], getv("damage"))
...
# ==========================================
# EXPORT FULL MRR PACKAGE (REPORT + DOCS)
# ==========================================

def _make_divider_page_pdf_bytes(title: str, subtitle: str = "") -> bytes:
    buf = BytesIO()
    ...
    if subtitle:
        ...
        c.drawCentredString(w/2, h-150, subtitle)
    c.setFont("Helvetica", 9)
    c.setFillColorRGB(0.35, 0.35, 0.35)
    c.drawCentredString(w/2, 40, "Generated by Inspection ERP")
    c.showPage()
    c.save()
    buf.seek(0)
    return buf.read()

def _image_file_to_pdf_bytes(img_path: str) -> bytes:
...
def _unsupported_file_placeholder_pdf_bytes(filename: str) -> bytes:
    return _make_divider_page_pdf_bytes(
        "Attachment not embedded",
        f"{filename} (download separately)",
    )

def _file_path_to_pdf_bytes(path: str) -> bytes:
    """
    Convert a file to PDF bytes if possible.
    - PDFs are returned as-is
    - Images are wrapped into a 1-page A4 PDF
    - Other files become a placeholder page (so auditors know something exists)
    """
    if not path or not os.path.exists(path):
        return _unsupported_file_placeholder_pdf_bytes(os.path.basename(path or "missing-file"))
    ...
    if ctype.startswith("image/") or ext in [".png", ".jpg", ".jpeg", ".webp"]:
        return _image_file_to_pdf_bytes(path)

    return _unsupported_file_placeholder_pdf_bytes(os.path.basename(path))
...
def _merge_pdf_bytes(parts: list[bytes]) -> bytes:
    ...
    for b in parts:
        try:
            ...
            # if something is corrupted, insert placeholder
            ph = _unsupported_file_placeholder_pdf_bytes("Corrupted/Unreadable attachment")
            r2 = PdfReader(BytesIO(ph))
            for p in r2.pages:
                writer.add_page(p)
...
async def create_shipment_inspection(
    ...
    # Auto-assign shipment documents uploaded BEFORE the inspection was created.
    # (PO stays ticket-level; everything else becomes part of this new shipment.)
    pending_docs = session.exec(
        select(MrrDocument).where(
            (MrrDocument.ticket_id == lot_id)
            & (MrrDocument.inspection_id == None)
            & (MrrDocument.doc_type != "PO")
        )
    ).all()
    if pending_docs:
        for d in pending_docs:
            d.inspection_id = int(insp.id)
            session.add(d)
        session.commit()
